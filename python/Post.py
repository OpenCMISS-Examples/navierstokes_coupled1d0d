...
def Post(nodes):
    ...
    DYNAMIC_SOLVER_NAVIER_STOKES_STOP_TIME      = 2000.0
    ...
    for node in nodes:
        ...
        for x in range(0,int(DYNAMIC_SOLVER_NAVIER_STOKES_STOP_TIME/DYNAMIC_SOLVER_NAVIER_STOKES_TIME_INCREMENT),100):
            ...
            for i in range(0,len(outputLINE)):
                if outputLINE[i].split() == ['Node:', str(node)]:
                    ...
                    Time = x*DYNAMIC_SOLVER_NAVIER_STOKES_TIME_INCREMENT/1000
